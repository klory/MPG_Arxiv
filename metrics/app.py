...
def load_models(ckpt_path, classifier_path, device):
    ...
    return label_encoder, g_ema, classifier
...
def main():
    st.set_page_config(page_title='MPG Demo', layout="wide")
    st.title('MPG: A Multi-ingredient Pizza Image Generator with Conditional StyleGANs')
    txt = '''
    | [Back to FoodAI](http://foodai.cs.rutgers.edu) | [Paper](https://arxiv.org/abs/2012.02821) | [Video](https://youtu.be/x3XKXMd1oC8) |[Code](https://github.com/klory/MPG_Arxiv) | [News from vice.com](https://www.vice.com/en/article/3anamk/ai-generated-pizzas-do-not-exist)|
    |---|---|---|---|---|
    ...
    Below is an interactive demo for **interpolation between two generated images**, \
        ...
        Check the [video](https://youtu.be/x3XKXMd1oC8) for more details.
    '''
    st.markdown(txt)
    ...
    ckpt_path = f'{ROOT}/mpg/runs/qo6nm72k/280000.pt'
    classifier_path = f'{ROOT}/ingr_classifier/runs/pizza10/1t5xrvwx/batch5000.ckpt'
    device='cuda'
    label_encoder, generator, classifier = load_models(ckpt_path, classifier_path, device)
    ...
    with st.spinner('Generating new images...'):
        ...
        ingr_from_src2 = mid_col.slider('Ingredients from Image B', min_value=0.0, max_value=1.0, value=0.5, step=0.01)
