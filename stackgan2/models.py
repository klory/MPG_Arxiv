...
class EMB_NET(nn.Module):
    def __init__(self):
        super(EMB_NET, self).__init__()
        self.t_dim = text_dimension
        self.ef_dim = embedding_dim
        self.fc = nn.Linear(self.t_dim, self.ef_dim * 2, bias=True)
        self.relu = GLU()

    def forward(self, text_embedding):
        out = self.relu(self.fc(text_embedding))
        return out, out, out
...
class G_NET(nn.Module):
    ...
    def define_module(self):
        if self.ca and self.b_condition:
            self.ca_net = CA_NET(cuda=self.cuda)
        elif not self.ca and self.b_condition:
            self.ca_net = EMB_NET()
...
if __name__ == '__main__':
    ...
    print('test G')
    G = G_NET(cuda=True, gf_dim=64, z_dim=100, r_num=2, levels=3, b_condition=True, ca=True).to(device)
    bs = 5
    z = torch.randn(bs, z_dim).to(device)
    txt_embedding = torch.randn(bs, text_dimension).to(device)
    fakes, mu, var = G(z, txt_embedding)
    for fake in fakes:
        print(fake.shape)
    ...
    for i, D in enumerate(Ds):
        out_cond, out_uncond = D(fakes[i], mu)
        print(i, out_cond.shape, out_uncond.shape)
